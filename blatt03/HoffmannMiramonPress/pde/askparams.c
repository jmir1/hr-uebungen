...
void AskParams(struct options *options, int argc, char **argv) {
    ...
    if (argc < 2) {
        ...
        } while ((options->method < METH_GAUSS_SEIDEL) ||
        ...
        } while ((options->termination < TERM_PREC) ||
        ...
        if (options->termination == TERM_PREC) {
            ...
            } while ((options->term_precision < 1e-20) ||
            ...
            } while ((options->term_iteration < 1) ||
                     (options->term_iteration > MAX_ITERATION));
        ...
        if (strcmp("help", (char *)argv[1]) == 0 ||
            strcmp("-h", (char *)argv[1]) == 0 || strcmp("-?", (char *)argv[1]) == 0 ||
            strcmp("?", (char *)argv[1]) == 0 || argc < 7) {
            printf("\nUsage:\n");
